...
def main():
    ...
    parser.add_argument('--output', default='ongeki_rating.json', help='输出JSON文件名')
    ...
    if save_success and args.excel:
        try:
            from b50_converter import convert_to_excel
            excel_file = args.output.replace('.json', '.xlsx')
            logger.info(f"转换为Excel文件: {excel_file}")
            convert_to_excel(args.output, excel_file)
            logger.info(f"Excel文件已生成: {excel_file}")
        except ImportError:
            logger.error("无法导入b50_converter模块，跳过Excel转换")
        except Exception as e:
